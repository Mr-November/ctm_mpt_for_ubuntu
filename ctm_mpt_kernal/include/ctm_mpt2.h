...
public:
    ...
    Eigen::Matrix<float, N_DIM, 1> pid(const Eigen::Matrix<float, N_DIM, 1> err);
...
private:
    ...
    Eigen::Matrix<float, N_DIM, 1> err_sum_;
    Eigen::Matrix<float, N_DIM, 1> err_last_;
...
private:
    ...
    Eigen::Matrix<float, N_DIM, N_DIM> jacobian3cc(const float L1 = 256,
                                                    const float L2 = 256,
                                                    const float L3 = 256);
    
    // State variables.
    float torque[N_CABLE] = {0.0};
    ...
    // Body transformation.
    Eigen::Matrix4f Tb = Eigen::Matrix4f::Identity();//Zero(4, 4);

    // Desired transformation.
    Eigen::Matrix4f Td = Eigen::Matrix4f::Identity();//Zero(4, 4);

    // Position PID controller.
    CtrlPID position_controller {0.1, 0.0, 0.0, 1000.0};
    ...
    Eigen::Matrix4f invT_zero = Eigen::Matrix4f::Zero();
    ...
    // Parameters.
    // Motor position.
    const uint8_t ID_MOTOR[N_MOTOR] = {1, 2, 3, 4, 5, 6, 7, 8, 9};
    ...
    const Eigen::Matrix<float, N_DIM, N_CABLE> U_DELTA {
    ...
    const Eigen::Matrix<float, N_DIM, N_DIM> C_DELTA {
    ...
    const float TORQUE_PERMITTED[N_MOTOR] = {5.0, 5.0, 5.0, 5.0, 5.0, 5.0, 5.0, 5.0, 5.0};

    const float TORQUE_OFFSET[N_MOTOR] = {-0.318507f, -0.792682f, -0.344987f,
    ...
    const float RESOLUTION[N_MOTOR] = { -200000/(7*_PI),  -200000/(7*_PI),  -200000/(7*_PI),
                                       -200000/(21*_PI), -200000/(21*_PI), -200000/(21*_PI),
                                       -200000/(21*_PI), -200000/(21*_PI), -200000/(21*_PI)};

    // Velocities. Unit: Hz.
    const int32_t VEL[N_MOTOR] = {12000, 12000, 12000,
                                   4000,  4000,  4000,
                                   4000,  4000,  4000};
    
    // Accelerations. Unit: Hz/s.
    const uint32_t ACC_START[N_MOTOR] = {1800000, 1800000, 1800000,
                                          600000,  600000,  600000,
                                          600000,  600000,  600000};

    const uint32_t ACC_BRAKE[N_MOTOR] = {1800000, 1800000, 1800000,
                                          600000,  600000,  600000,
                                          600000,  600000,  600000};
};
