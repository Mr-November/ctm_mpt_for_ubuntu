...
VectorXd update_L(VectorXd L,VectorXd jnt,MatrixXd Tfb,bool clear);
int main (int argc, char** argv)
{
    ...
    if (ros::ok())
	{
        ...
        while (cur > tot)
		{
			...
			L_p = update_L(L_p,jnt,T_fb,false);


			cout<<"TpTfb"<<endl;
			cout<<T_p<<endl<<T_fb<<endl;
			cout<<"L_p"<<endl;
			cout<<L_p.transpose()<<endl;
			getchar();
		}
		fin.clear();
		fin.close();
...
/* 			if (cur < 3)
			...
			{
				J_1 = invA(jacobian3cc(xi_1,L_p(0)+L_p(1)+L_p(2),L_p(3)+L_p(4)+L_p(5),L_p(6)+L_p(7)+L_p(8)));
				dxi = J_1 * dw_n;
				xi_n = xi_1 + dxi;
				xi_1 = xi_n;
			}
			...
			jnt = xi2jnt(xi_n,L_p(0)+L_p(1)+L_p(2),L_p(3)+L_p(4)+L_p(5),L_p(6)+L_p(7)+L_p(8));
			Error.angular.x = err[0];
			Error.angular.y = err[1];
			Error.angular.z = err[2];

			Error.linear.x = err[3];
			Error.linear.y = err[4];
			Error.linear.z = err[5];
...
{	
	Matrix3d R[9];
	MatrixXd C(3,10);
	Vector3d d,Y = Tfb.block(0,3,3,1);
	d<<0,0,1;
	cout<<d<<endl;
	double tx,ty;
	Matrix3d Rotx,Roty;
	static MatrixXd K = MatrixXd::Zero(10,3);
	static MatrixXd P = 10*MatrixXd::Ones(10,10);
	for (size_t i = 0;i<9;i++)
	{
		if (i%2 == 0)
		{
			ty = jnt(2*i);
			tx = jnt(2*i+1);
			Rotx << 1,0,0,cos(tx),-sin(tx),0,0,sin(tx),cos(tx);
		...
		C.block(0,i,3,1) = R[i]*d;
	}
	C.block(0,9,3,1) = d;
	if (clear)
	{
		K = MatrixXd::Zero(10,3);
		P = 10*MatrixXd::Ones(10,10);
	}

	L = L+K*(Y-C*L);
	K = P*C.transpose()*invA(MatrixXd::Identity(3,3)+C*P*C.transpose());
